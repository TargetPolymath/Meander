...
def attn(x, scope, n_state, *, past, hparams):
    ...
    def multihead_attn(q, k, v):
        ...
        # print(f"v Shape hH_a 0: {v.shape}")
        w = tf.matmul(q, k, transpose_b=True)
        # print(f"W Shape hH_a -1: {w.shape}")
        w = w * tf.math.rsqrt(tf.cast(v.shape[-1], w.dtype))
        # print(f"W Shape hH_a 0: {w.shape}")
...
def model(hparams, X, past=None, scope='model', reuse=tf.compat.v1.AUTO_REUSE):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        # try:
        #     # print(f"Pasts shape (model 181) {len(pasts)}/{pasts[0].shape}")
        # except:
        #     print("Pasts is None's")
        
        attentions = []
        assert len(pasts) == hparams.n_layer
        for layer, past in enumerate(pasts):
            # print("Past Layers Flag")
            h, present, attention = block(h, 'h%d' % layer, past=past, hparams=hparams)
            # print(f"Model Attention Shape: {attention.shape}")
            if layer == 10:
                tf.compat.v1.add_to_collection('checkpoints', h)
            presents.append(present)
            attentions.append(attention)
        # print(f"Model presents shape: {len(presents)}/{presents[0]}")
        results['present'] = tf.stack(presents, axis=1)
        results['attentions'] = tf.stack(attentions, axis=1)
       

        # [batch, layer, heads, dest, src]
        # print(f"Model Attention Stacked: {results['attentions']}")
        h = norm(h, 'ln_f')
